...
namespace Day10
{
    public class Program
    {
        ...
        {
            var originalSkyLights = Input.Select(skylight => new SkyLight(skylight)).ToList();

            Part1(originalSkyLights);
        }

        public static void Part1(List<SkyLight> skyLights)
        {
            var iterations = new Dictionary<int, int>();
            for (var i = 0; i < 20000; i++)
            {
                (Point min, Point max) = CalculateBoundingBoxAtTime(CalculateNewPositionsAtTime(skyLights, i));
        ...
        {
            return skyLights.Select(skylight => skylight.CalculatePositionAtTime(time)).ToList();
        }
        ...
        private static void DrawPositionsAtTime(IEnumerable<SkyLight> skyLights, int time)
        {
            var newPositions = CalculateNewPositionsAtTime(skyLights, time);
    ...
    {
        public static Point CalculatePositionAtTime(this SkyLight skyLight, int time)
        {
            return new Point(skyLight.Position.X + skyLight.Velocity.X * time, skyLight.Position.Y + skyLight.Velocity.Y * time);
        }
    }
}
