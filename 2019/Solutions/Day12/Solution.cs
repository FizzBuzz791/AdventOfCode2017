...
namespace Solutions.Day12
{
	public class Solution : BaseSolution<List<Moon>>, ISolvable
	{
		...
		{
			var moons = Input.Select(m => new Moon(m)).ToList();
		...
		{
			var xAxis = Input.Select(m => new[] { m.Position.X, m.Velocity.X }).ToArray();
			var xAxisSteps = FindStepsToHalfway(xAxis);

			var yAxis = Input.Select(m => new[] { m.Position.Y, m.Velocity.Y }).ToArray();
			var yAxisSteps = FindStepsToHalfway(yAxis);

			var zAxis = Input.Select(m => new[] { m.Position.Z, m.Velocity.Z }).ToArray();
			var zAxisSteps = FindStepsToHalfway(zAxis);

			var lcm = LCM(new[] { xAxisSteps * 2, yAxisSteps * 2, zAxisSteps * 2 });
			return $"Part 2: {lcm}";
		}
		...
		{
			var original = axis.Select(x => new[] { x[0], x[1] }).ToArray();
