...
namespace AdventOfCode.Year2019.Day16
{
    public class Solution : BaseSolution<string>, ISolvable
    {
        ...
        {
            IEnumerable<int> output = FlawedFrequencyTransmission(Input);
        ...
        private static string DecodeSignal(string input)
        {
            ...
            List<int> list = signal.Select(c => int.Parse(c.ToString())).ToList();
        ...
        private static IEnumerable<int> FlawedFrequencyTransmission(string input)
        {
            ...
            List<int> list = input.Select(c => int.Parse(c.ToString())).ToList();
