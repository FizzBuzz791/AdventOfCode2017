...
namespace AdventOfCode.Year2019.Day3
{
    public class Solution : BaseSolution<string>, ISolvable
    {
        ...
        {
            IEnumerable<Step> crossovers = FindCrossovers(Input);
        ...
        {
            string[] wire1Instructions = input.Split("\n")[0].Split(",");
            string[] wire2Instructions = input.Split("\n")[1].Split(",");

            Dimensions wire1Dimensions = ProcessInstructions(wire1Instructions);
            Dimensions wire2Dimensions = ProcessInstructions(wire2Instructions);
        ...
        private static Dimensions ProcessInstructions(IEnumerable<string> wireInstructions)
        {
            ...
            return new Dimensions { MinX = minX, MinY = minY, MaxX = maxX, MaxY = maxY };
        }
        ...
        private static void LayWires(IEnumerable<string> wireInstructions, Step?[,] grid, bool isWire1)
        {
            ...
            foreach (var instruction in wireInstructions)
            {
                ...
                while (distance > 0)
                {
                    ...
                    grid[currentX, currentY] ??= new Step { Location = new Point(currentX, currentY) };
