...
namespace AdventOfCode.Year2019.Day15
{
    public class RepairDroidControlSystem
    {
		...
		public static Point Origin => new(Midpoint, Midpoint);
		public Point Destination { get; private set; }
		private Point DroidLocation { get; set; } = new(Midpoint, Midpoint);
		...
		public void Explore()
		{
			...
			do
			{
				Instruction instruction = CalculateMoveDirection(lastInstruction, lastOutput);
		...
		public IEnumerable<Node> FindPath(Point source)
		{
			...
			Node? currentNode = nodes.SingleOrDefault(n => n.Location == source);
			if (currentNode != null)
			{
		...
		{
			Point featureLocation = CalculateLocation(instruction);
		...
		{
			Point newDroidLocation = CalculateLocation(instruction);
		...
		private static Instruction CalculateMoveDirection(Instruction lastDirection, Response lastOutput)
		{
			if (lastOutput == Response.Wall)
			{
				...
				return lastDirection switch
				{
					...
					_ => throw new ArgumentOutOfRangeException(nameof(lastDirection), $"Unexpected direction: {lastDirection}")
                };
			}
